...
_PORT_SENSORS: list[_PortSensorDesc] = [
    ...
    _PortSensorDesc(
        ...
        value_fn=lambda p: round(p.rx_octets / 1_048_576, 2) if p.rx_octets is not None else None,
    ),
    _PortSensorDesc(
        key="tx",
        ...
        value_fn=lambda p: round(p.tx_octets / 1_048_576, 2) if p.tx_octets is not None else None,
    ),
    _PortSensorDesc(
        key="rx_rate",
        label="débit RX",
